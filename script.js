...
function showQuestionsForDay(dayNumber) {
  ...
  todaysQuestions.forEach((q, i) => {
    ...
    const questionText = `<strong>Q${start + i + 1}:</strong> ${q.question} <br><em>${q.category}</em>`;
    const answerText = `<div class="answer">${q.answer}</div>`;
    const toggleBtn = `<button class="toggle-answer" style="margin-top: 0.5rem; background: #66bb6a; border: none; color: white; padding: 0.4rem 0.8rem; border-radius: 0.4rem; cursor: pointer;">Show Answer</button>`;

    div.innerHTML = questionText + toggleBtn + answerText;
    container.appendChild(div);
  });

    document.querySelectorAll('.toggle-answer').forEach((btn, i) => {
